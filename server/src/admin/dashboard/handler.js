...
const getAllResources = async (context) => {
    const allowedResourcesOnly = context._admin.resources.filter(
        (resource) => resource._decorated.actions.show.action.isAccessible(context));
...
const getGroupResources = (navGroup, resources) => {
    const navGroupResources = resources.filter(
        (resource) => {
...
const getResourcesByGroups = async (context) => {
    ...
    return Object.values(navigationGroups).map(
        (navGroup) => ({
            navGroup,
...
const getAdmin = ({ currentAdmin: { _id, fullName, email, isSuperuser, role } }) => ({
    _id, fullName, email, isSuperuser, role
});
